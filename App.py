...
def budgetRevenueRelationship(collection1, collection2):
    ...
    project = {
        "$project": {"_id": 0, "country": 1, "GDP ($ per capita)": 1,
        ...
        if x["MovieInfo"]:
            ...
            for y in x["MovieInfo"]:
                if (len(y["budget"]) != 0 and len(y["gross"]) != 0):
                    temp = float(y["gross"]) / float(y["budget"])
            ...
            if count != 0:
                avg = total / count
                revGross[x["country"]] = avg
                gdps.append(x["GDP ($ per capita)"])

    print(revGross)
    print(gdps)

    figure = plt.figure(figsize=(15, 10))
    plt.scatter( gdps, list(revGross.values()), color='blue')

    plt.xlabel("GrossRevRatio")
    plt.ylabel("GDP (per capita)")
    plt.title("Average Gross Revenue Ratio vs GDP")
    plt.show()
